...
import rospy
import time
from std_msgs.msg import String,Bool  #String message type for publishing

#Map function from arduino
def map(x,in_min,in_max,out_min,out_max):
    return (x-in_min)*(out_max-out_min)/(in_max-in_min)+out_min

#Initializes serial communication (if there is an error 'no suchfile or directory$
...
#Declares that this node is publishing to the 'joystick_topic' topic using the me$
pub = rospy.Publisher('joystick_topic', String, queue_size = 10)

#Variable for the ultrasonic detection
distancia = 0

#Function that will execute every time a boolean value from the ultrasonic is rec$
def callback(data):
    global distancia
    distancia = data.data

#Decalres that this node subscribes to the distance_topic whose message type is a$
rospy.Subscriber("distance_topic", Bool, callback)

#Tells rospy the name of the node
rospy.init_node('RjoystickU', anonymous = True)
